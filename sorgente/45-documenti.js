...
async function apriAssistenteDocumento(file){
  ...
  const ridisegna=canvas=>{
    canvas.width=cw;canvas.height=ch;
    const ctx=canvas.getContext('2d');
    ctx.clearRect(0,0,cw,ch);ctx.drawImage(img,0,0,cw,ch);
    for(const c of st.coperture){ ctx.fillStyle=c.colore; ctx.fillRect(c.x/100*cw,c.y/100*ch,c.w/100*cw,c.h/100*ch); }
    if(st.timbro.attivo&&stampaImg){ const w=cw*0.22*(st.timbro.scala/100),hh=w*(stampaImg.height/stampaImg.width); ctx.drawImage(stampaImg,st.timbro.x/100*cw-w/2,st.timbro.y/100*ch-hh/2,w,hh); }
    if(st.testo.attivo){ ctx.fillStyle='#000';ctx.font=st.testo.dimensione+'px Arial,sans-serif';ctx.textBaseline='top'; (st.testo.testo||'').split('\n').forEach((riga,i)=>ctx.fillText(riga,st.testo.x/100*cw,st.testo.y/100*ch+i*(st.testo.dimensione+3))); }
  };
  const corpo=()=>html`<div class="griglia due">
    <div><canvas id="tela-assistente" style="width:100%;height:auto;border:1px solid var(--bordo);border-radius:var(--raggio);background:#fff"></canvas></div>
    <div>
      <div class="scheda mb-s"><h3>${icona('elimina','piccola')}Copri un'area (es. l'intestazione di un'altra azienda)</h3>
        <div class="campi"><div class="campo"><span class="etichetta-campo">X %</span><input type="number" id="ass-cop-x" value="5" min="0" max="100"></div><div class="campo"><span class="etichetta-campo">Y %</span><input type="number" id="ass-cop-y" value="2" min="0" max="100"></div><div class="campo"><span class="etichetta-campo">Largh. %</span><input type="number" id="ass-cop-w" value="90" min="1" max="100"></div><div class="campo"><span class="etichetta-campo">Alt. %</span><input type="number" id="ass-cop-h" value="12" min="1" max="100"></div></div>
        <button class="pulsante piccolo mt-s" id="ass-cop-agg">${icona('piu','piccola')}Aggiungi rettangolo bianco</button>
        <ul class="elenco-piatto piccolo mt-s" id="ass-cop-lista"></ul>
      </div>
      <div class="scheda mb-s"><h3><label class="spunta"><input type="checkbox" id="ass-timbro-on">${icona('firma','piccola')}Timbro e firma</label></h3>
        <div class="campi"><div class="campo"><span class="etichetta-campo">X %</span><input type="number" id="ass-timbro-x" value="75" min="0" max="100"></div><div class="campo"><span class="etichetta-campo">Y %</span><input type="number" id="ass-timbro-y" value="85" min="0" max="100"></div><div class="campo"><span class="etichetta-campo">Dimensione %</span><input type="number" id="ass-timbro-s" value="100" min="20" max="300"></div></div>
      </div>
      <div class="scheda"><h3><label class="spunta"><input type="checkbox" id="ass-testo-on">${icona('persona','piccola')}Dati legale rappresentante</label></h3>
        <textarea id="ass-testo-testo" rows="4" class="mb-s">${testoLegaleRappresentante()}</textarea>
        <div class="campi"><div class="campo"><span class="etichetta-campo">X %</span><input type="number" id="ass-testo-x" value="8" min="0" max="100"></div><div class="campo"><span class="etichetta-campo">Y %</span><input type="number" id="ass-testo-y" value="85" min="0" max="100"></div><div class="campo"><span class="etichetta-campo">Dimensione testo</span><input type="number" id="ass-testo-d" value="16" min="8" max="60"></div></div>
      </div>
    </div>
  </div>`;
  const risultato=await dialogo({titolo:'Assistente documento esterno · '+file.name,enorme:true,corpo:corpo(),
    pulsanti:[{testo:'Chiudi',valore:null},{testo:'Stampa',fn:v=>{const c=v.querySelector('#tela-assistente');const u=c.toDataURL('image/png');const w=window.open('');w.document.write('<img src="'+u+'" style="max-width:100%" onload="print()">');return false}},{testo:'Condividi',fn:async v=>{const c=v.querySelector('#tela-assistente');const b=await canvasABlob(c,'image/png',1);await condividiOScarica(b,file.name.replace(/\.[a-z0-9]+$/i,'')+' - modificato.png');return false}},{testo:'Salva nell\'archivio',classe:'primario',primario:true,fn:async v=>{const c=v.querySelector('#tela-assistente');const b=await canvasABlob(c,'image/jpeg',0.92);return {blob:b}}}],
    alMontaggio:v=>{
      const canvas=v.querySelector('#tela-assistente');
      const agg=()=>ridisegna(canvas);
      agg();
      const lista=()=>{v.querySelector('#ass-cop-lista').innerHTML=st.coperture.map((c,i)=>html`<li>Rettangolo ${i+1} (${Math.round(c.x)}%,${Math.round(c.y)}%) <button class="pulsante piccolo icona" data-tog="${i}" aria-label="Togli">${icona('chiudi','piccola')}</button></li>`).map(x=>x.s).join('');tutti('[data-tog]',v).forEach(b=>b.onclick=()=>{st.coperture.splice(+b.dataset.tog,1);lista();agg()})};
      v.querySelector('#ass-cop-agg').onclick=()=>{st.coperture.push({x:+v.querySelector('#ass-cop-x').value||0,y:+v.querySelector('#ass-cop-y').value||0,w:+v.querySelector('#ass-cop-w').value||10,h:+v.querySelector('#ass-cop-h').value||10,colore:'#fff'});lista();agg()};
      lista();
      const campo=(id,ramo,chiave,num)=>{const el=v.querySelector(id);el.addEventListener('input',()=>{ramo[chiave]=num?(+el.value||0):el.value;agg()})};
      v.querySelector('#ass-timbro-on').addEventListener('change',e=>{st.timbro.attivo=e.target.checked;agg()});
      campo('#ass-timbro-x',st.timbro,'x',true);campo('#ass-timbro-y',st.timbro,'y',true);campo('#ass-timbro-s',st.timbro,'scala',true);
      v.querySelector('#ass-testo-on').addEventListener('change',e=>{st.testo.attivo=e.target.checked;agg()});
      campo('#ass-testo-x',st.testo,'x',true);campo('#ass-testo-y',st.testo,'y',true);campo('#ass-testo-d',st.testo,'dimensione',true);
      campo('#ass-testo-testo',st.testo,'testo',false);
    }});
  if(risultato&&risultato.blob){
    const fileFinale=new File([risultato.blob],file.name.replace(/\.[a-z0-9]+$/i,'')+' - modificato.jpg',{type:'image/jpeg'});
    dialogoDocumento({},[fileFinale]);
  }
}
...
const SEGNAPOSTO_GRUPPI=[
  ['Azienda','azienda.ragioneSociale, azienda.sede, azienda.piva, azienda.cf, azienda.pec, azienda.telefono, azienda.inail, azienda.rea, azienda.email'],
  ['Legale rappresentante','legale.nome, legale.natoA, legale.natoIl, legale.residenza, legale.cf'],
  ['Cantiere (se scelto in fase di compilazione)','cantiere.nome, cantiere.descrizione, cantiere.indirizzo, cantiere.comune, cantiere.cap, cantiere.lavorazioni, cantiere.lavorazioniElenco'],
  ['Committente e affidataria','committente.nome, committente.indirizzo, affidataria.nome, affidataria.indirizzo'],
  ['Figure della sicurezza','cse.nome, rspp.nome, rls.nome, medico.nome, preposto.nome, preposto.cf, preposto.natoIl, preposto.corsoData'],
  ['Operai del cantiere','operai.elenco (lista puntata), operai.tabella (tabella con dati anagrafici), addettiAntincendio.elenco, addettiPrimoSoccorso.elenco'],
  ['Altro','oggi (data di compilazione), luogoData (riquadro luogo/data/firma)'],
];
function dialogoModello(m){
  ...
  const segnapostoHtml=SEGNAPOSTO_GRUPPI.map(([g,voci])=>html`<div class="mb-s"><b class="piccolo">${g}</b><br><span class="mono piccolo">${voci}</span></div>`);
  return dialogoModulo(nuovo?'Nuovo modello':'Modello: '+m.nome,[{nome:'nome',etichetta:'Nome del documento',obbligatorio:true,largo:true},{nome:'firmatario',etichetta:'Dicitura firmatario'},{nome:'cartaIntestata',tipo:'spunta',testo:'Su carta intestata'},{nome:'firma',tipo:'spunta',testo:'Va firmato'},{nome:'timbro',tipo:'spunta',testo:'Va timbrato'},{nome:'soloConSubappalto',tipo:'spunta',testo:'Solo se ci sono subappaltatori'},{nome:'testo',etichetta:'Testo (Markdown semplice: # titolo, ## sottotitolo, - elenco, **grassetto**, tabelle con |)',tipo:'textarea',righe:18,largo:true,obbligatorio:true}],m,{intro:html`<p class="piccolo secondario">Scrivi il testo normalmente e usa un segnaposto fra doppie graffe (es. <span class="mono">{{legale.nome}}</span>) dove vuoi che si riempia da solo. Quello che manca compare in rosso, non si inventa mai. Segnaposto disponibili:</p><div class="riquadro">${segnapostoHtml}</div>`,pulsantiExtra:nuovo?[]:[{testo:'Elimina',classe:'pericolo',sinistra:true,fn:async()=>{if(await conferma('Eliminare il modello «'+m.nome+'»?',{pericolo:true,ok:'Elimina'})){esegui('Eliminato modello',s=>{s.modelli.dichiarazioni=s.modelli.dichiarazioni.filter(x=>x.id!==m.id)});return null}return false}}]}).then(v=>{if(!v)return;if(nuovo)esegui('Nuovo modello '+v.nome,s=>{s.modelli.dichiarazioni.push(Object.assign({id:nuovoId('mod')},v))});else esegui('Modificato modello '+v.nome,s=>{Object.assign(s.modelli.dichiarazioni.find(x=>x.id===m.id),v)})});
}
