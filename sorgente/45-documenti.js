...
VISTE.documenti=function(r){
  ...
  const testa=html`<div class="testata"><div><h1>Documenti</h1><div class="sotto">${stato.file.filter(f=>!f.cestinato).length} file · ${fPeso(somma(stato.file.filter(f=>!f.cestinato),f=>f.dimensione))} nell'archivio</div></div><div class="azioni"><button class="pulsante" data-azione="documento-nuovo">${icona('allega')}Aggiungi documento</button><a class="pulsante" href="#/documenti/caricamento">${icona('cartella')}Caricamento cartelle</a></div></div>
  ${linguette('documenti',[{id:'archivio',testo:'Archivio',icona:'documenti'},{id:'buste',testo:'Buste paga',icona:'busta-paga'},{id:'peso',testo:'Peso',icona:'peso'},{id:'modelli',testo:'Modelli',icona:'firma'},{id:'generati',testo:'Generati',icona:'stampa'},{id:'caricamento',testo:'Caricamento',icona:'cartella'}].map(x=>({...x,href:'#/documenti/'+(x.id==='archivio'?'':x.id)})),sotto)}`;
  const corpo=sotto==='buste'?vistaBustePaga(r):sotto==='peso'?vistaPeso():sotto==='modelli'?vistaModelli(r):sotto==='generati'?vistaGenerati():sotto==='caricamento'?vistaCaricamento():vistaArchivio();
...
function vistaArchivio(){
  ...
  if(f.peso==='pesanti') docs=docs.filter(x=>x.peso>1024*1024); if(f.peso==='senza') docs=docs.filter(x=>!(x.d.file||[]).length);
...
function vistaModelli(r){
  ...
  ${sotto==='dichiarazioni'?html`<div class="scheda"><h3>Modelli di dichiarazione <span class="azioni"><button class="pulsante piccolo" data-azione="modello-nuovo">${icona('piu')}Nuovo</button></span></h3><p class="secondario piccolo">Clicca <b>Compila</b> per averla pronta da stampare in pochi secondi: si riempie da sola con i dati dell'azienda e, se scegli un cantiere, anche i suoi. Clicca sulla riga per modificare il testo del modello (segnaposto fra doppie graffe, es. <span class="mono">{{cantiere.nome}}</span>).</p>${tabella({righe:stato.modelli.dichiarazioni,onRiga:m=>dialogoModello(m),colonne:[{chiave:'nome',titolo:'Documento',principale:true},{chiave:'ci',titolo:'Carta intestata',formatta:m=>m.cartaIntestata?'sì':'no'},{chiave:'f',titolo:'Firma',formatta:m=>m.firma?'sì':'no'},{chiave:'t',titolo:'Timbro',formatta:m=>m.timbro?'sì':'no'},{chiave:'s',titolo:'Firmatario',formatta:m=>m.firmatario||''},{chiave:'az',titolo:'',classe:'azioni',formatta:m=>html`<button class="pulsante piccolo primario" data-azione="dichiarazione-compila" data-modello="${m.id}">${icona('stampa','piccola')}Compila</button>`}]})}</div>`:''}
