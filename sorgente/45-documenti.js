...
function vistaModelli(r){
  ...
  ${sotto==='dichiarazioni'?html`<div class="scheda"><h3>Modelli di dichiarazione <span class="azioni"><button class="pulsante piccolo" data-azione="modello-nuovo">${icona('piu')}Nuovo</button></span></h3><p class="secondario piccolo">Il testo usa segnaposto fra doppie graffe (es. <span class="mono">{{cantiere.nome}}</span>) che si riempiono dai dati del cantiere. Elenco completo nell'editor.</p>${tabella({righe:stato.modelli.dichiarazioni,onRiga:m=>dialogoModello(m),colonne:[{chiave:'nome',titolo:'Documento',principale:true},{chiave:'ci',titolo:'Carta intestata',formatta:m=>m.cartaIntestata?'sì':'no'},{chiave:'f',titolo:'Firma',formatta:m=>m.firma?'sì':'no'},{chiave:'t',titolo:'Timbro',formatta:m=>m.timbro?'sì':'no'},{chiave:'s',titolo:'Firmatario',formatta:m=>m.firmatario||''}]})}</div>`:''}
...
function dialogoModello(m){
  ...
  const segnaposto='azienda.ragioneSociale, azienda.sede, azienda.piva, azienda.cf, azienda.pec, azienda.telefono, azienda.inail, azienda.rea, legale.nome, legale.natoA, legale.natoIl, legale.residenza, legale.cf, cantiere.nome, cantiere.descrizione, cantiere.indirizzo, cantiere.comune, cantiere.cap, cantiere.lavorazioni, cantiere.lavorazioniElenco, committente.nome, committente.indirizzo, affidataria.nome, affidataria.indirizzo, cse.nome, rspp.nome, rls.nome, medico.nome, preposto.nome, preposto.cf, preposto.natoIl, preposto.corsoData, operai.elenco, operai.tabella, addettiAntincendio.elenco, addettiPrimoSoccorso.elenco, oggi, luogoData';
  return dialogoModulo(nuovo?'Nuovo modello':'Modello: '+m.nome,[{nome:'nome',etichetta:'Nome del documento',obbligatorio:true,largo:true},{nome:'firmatario',etichetta:'Dicitura firmatario'},{nome:'cartaIntestata',tipo:'spunta',testo:'Su carta intestata'},{nome:'firma',tipo:'spunta',testo:'Va firmato'},{nome:'timbro',tipo:'spunta',testo:'Va timbrato'},{nome:'soloConSubappalto',tipo:'spunta',testo:'Solo se ci sono subappaltatori'},{nome:'testo',etichetta:'Testo (Markdown semplice: # titolo, ## sottotitolo, - elenco, **grassetto**, tabelle con |)',tipo:'textarea',righe:18,largo:true,obbligatorio:true}],m,{intro:html`<p class="piccolo secondario">Segnaposto: <span class="mono">${segnaposto}</span></p>`,pulsantiExtra:nuovo?[]:[{testo:'Elimina',classe:'pericolo',sinistra:true,fn:async()=>{if(await conferma('Eliminare il modello «'+m.nome+'»?',{pericolo:true,ok:'Elimina'})){esegui('Eliminato modello',s=>{s.modelli.dichiarazioni=s.modelli.dichiarazioni.filter(x=>x.id!==m.id)});return null}return false}}]}).then(v=>{if(!v)return;if(nuovo)esegui('Nuovo modello '+v.nome,s=>{s.modelli.dichiarazioni.push(Object.assign({id:nuovoId('mod')},v))});else esegui('Modificato modello '+v.nome,s=>{Object.assign(s.modelli.dichiarazioni.find(x=>x.id===m.id),v)})});
}
