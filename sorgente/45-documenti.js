...
function vistaArchivio(){
  ...
  const problemi=rr=>rr.filter(x=>documentoCritico(x.info)).length;
  const COLONNE=[
  ...
  ${docs.length?chiavi.map(c=>{const rr=gruppi.get(c);const pb=problemi(rr);return html`<details class="ck-sezione" ${pb||filtroAttivo?'open':''}><summary><span class="freccia">${icona('destra','piccola')}</span><b class="spazio">${CATEGORIE_TIPO_DOC[c]||c}</b>${pb?html`<span class="pillola scaduto piccolo">${pb} da sistemare</span>`:html`<span class="pillola valido piccolo">${icona('ok','piccola')}a posto</span>`}<span class="conteggio">${rr.length}</span></summary><div class="ck-corpo">${tabella({id:'arch'+c,righe:rr,chiaveOrd:'stato',onRiga:x=>apriDocumento(x.d.id),classeRiga:x=>'riga-'+(x.info.silenziato?'riferimento':x.info.stato),colonne:COLONNE})}</div></details>`})
...
async function apriDocumento(docId){
  ...
  // si può tacere solo un documento che non blocca l'idoneità: quelli bloccanti vanno rinnovati
  const tacibile=(['scaduto','scadenza','pianificare'].includes(info.stato)||d.avvisoTaciuto)&&!(t&&t.bloccaIdoneita);
  const meta=html`<div class="campi" style="grid-template-columns:repeat(auto-fill,minmax(150px,1fr))">
    ...
    ${tacibile?html`<div class="campo largo"><div class="avviso-inline ${d.avvisoTaciuto?'':'attenzione'}">${icona(d.avvisoTaciuto?'silenzio':'attenzione')}<div class="corpo">${d.avvisoTaciuto?html`Questa scadenza <b>non viene segnalata</b>: non conta fra le cose da sistemare e non colora la scheda. La data resta scritta.`:html`Documento ${info.stato==='scaduto'?'scaduto':'in scadenza'}. Se in cantiere non lo chiedono e non vuoi vederlo fra le cose da sistemare, puoi smettere di farlo segnalare senza toglierne la data.`} <button class="pulsante piccolo" style="margin-left:6px" data-azione="documento-taci" data-id="${d.id}">${d.avvisoTaciuto?'Segnala di nuovo':'Non segnalare più'}</button></div></div></div>`:''}
  ...
  ${files.length?html`<div class="riga stretta mt" style="gap:8px;flex-wrap:wrap"><button class="pulsante piccolo" data-azione="doc-aggiungi-file" data-doc="${d.id}">${icona('carica','piccola')}Allega un altro file</button><button class="pulsante piccolo" data-azione="file-apri" data-id="${files[0].id}" title="Apre il file da solo, a tutta altezza">${icona('occhio','piccola')}Apri a tutto schermo</button></div>`:html`<div class="zona-drop mt" data-azione="doc-aggiungi-file" data-doc="${d.id}" data-drop-doc="${d.id}">${icona('carica')}Trascina qui la scansione o clicca per allegare un file</div>`}`;
  apriPannello({titolo:(t?t.nome:'Documento')+' · '+nomeSoggettoDoc(d),largo:!!files.length,corpo:meta,anteprima:files.length?'anteprima-doc':null,piede:html`<button class="pulsante primario" data-azione="documento-modifica" data-id="${d.id}">${icona('modifica')}Modifica</button><button class="pulsante" data-azione="documento-duplica" data-id="${d.id}" title="Crea il rinnovo: stesso tipo e soggetto, date nuove">${icona('aggiorna')}Rinnovo</button><span class="spazio"></span><button class="pulsante pericolo" data-azione="documento-elimina" data-id="${d.id}">${icona('elimina')}Elimina</button>`});
  if(files.length){const a=await htmlAnteprimaFile(files[0].id);const c=el('#anteprima-doc');if(c)c.innerHTML=a}
}
AZIONI['doc-anteprima-file']=async d=>{const c=el('#anteprima-doc');if(c){c.innerHTML=html`<div class="anteprima-nessuna">Caricamento…</div>`;c.innerHTML=await htmlAnteprimaFile(d.id)}};
...
AZIONI['documento-taci']=d=>{const doc=perId('documenti',d.id);const ora=!doc.avvisoTaciuto;esegui(ora?'Scadenza non più segnalata':'Scadenza segnalata di nuovo',s=>{const x=s.documenti.find(x=>x.id===d.id);if(ora)x.avvisoTaciuto=true;else delete x.avvisoTaciuto},{senzaRender:true});apriDocumento(d.id);render()};
...
function vistaModelli(r){
  ...
  ${sotto==='carta'?html`<div class="griglia due"><div class="scheda"><h3>Carta intestata</h3><p class="secondario piccolo">Righe di testo accanto al logo, su ogni pagina dei documenti.</p><ol>${(stato.azienda.cartaIntestata.righe||[]).map(x=>html`<li>${x}</li>`)}</ol><button class="pulsante" data-azione="carta-modifica">${icona('modifica')}Modifica righe</button>
    <div class="mt"><div class="etichetta-campo">Logo</div><div class="mt-s"><img src="${immagineAzienda('logo')}" alt="Logo" style="height:60px"></div><div class="mt-s"><button class="pulsante piccolo" data-azione="immagine-sostituisci" data-chiave="logo">${icona('carica','piccola')}Sostituisci</button>${stato.azienda.img_logo?html` <button class="pulsante piccolo" data-azione="immagine-ripristina" data-chiave="logo">Originale</button>`:''}</div></div></div>
