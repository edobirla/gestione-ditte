...
function schedaOrePersona(p){
  ...
  return html`<div class="scheda"><h3>Ore negli ultimi dodici mesi</h3>${tabella({righe,colonne:[
    ...
    {chiave:'ore',titolo:'Ore in griglia',num:true,formatta:r=>r.calc?fOre(r.calc.oreGriglia):html`<span class="silenzioso">—</span>`},
    {chiave:'dich',titolo:'Ore dichiarate',num:true,formatta:r=>r.calc&&r.calc.oreDichiarate!=null?html`${fOre(r.calc.oreDichiarate)}${r.calc.differenzaOre?html` <span class="piccolo ${r.calc.differenzaOre>0?'secondario':'da-compilare'}">(${r.calc.differenzaOre>0?'+':''}${fOre(r.calc.differenzaOre)})</span>`:''}`:html`<span class="silenzioso">—</span>`},
