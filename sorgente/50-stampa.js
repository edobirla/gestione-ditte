...
function bloccoFirma(opz){
  ...
  const src=opz.src?h(opz.src):'{{IMG:'+(opz.immagine||'firmaTimbro')+'}}';
  return html`<div class="blocco-firma"><div class="luogo">${a.indirizzo.comune||'Subbiano'} (${a.indirizzo.provincia||'AR'}), ${fData(opz.data||oggi())}</div><div class="firma"><div>${opz.etichetta||'Timbro e firma impresa/società'}</div><div class="linea">${opz.senzaFirma?'':grezzo('<img src="'+src+'" alt="">')}</div></div></div>`.s;
}
...
function contestoDichiarazione(c,opz){
  ...
  return (k)=>{
    if(k==='luogoData') return {blocco:bloccoFirma({data:opz.data,senzaFirma:opz.senzaFirma,etichetta:opz.etichettaFirma,src:opz.firmaSrc})};
...
function docDichiarazione(modello,c,opz){
  opz=opz||{};
  const blocchi=blocchiDaMarkdown(opz.testo!==undefined?opz.testo:modello.testo,contestoDichiarazione(c,{data:opz.data,senzaFirma:opz.senzaFirma!==undefined?opz.senzaFirma:!modello.firma,firmaSrc:opz.firmaSrc}));
...
AZIONI['dichiarazione-compila']=async d=>{
  ...
  await compilaDichiarazione(m,c,{data:d.data||oggi()});
};
// L'anteprima non salva niente: si guarda, si corregge quello che serve (testo, data, firma) e solo
// premendo "Salva" la dichiarazione entra fra i documenti del cantiere e la checklist la dà per
// fatta. Prima veniva registrata appena si apriva l'anteprima, anche se poi si chiudeva.
async function compilaDichiarazione(m,c,iniziale){
  const st={data:(iniziale&&iniziale.data)||oggi(),testo:m.testo,firmaSrc:null,firmaNome:'quella prevista dal modello',senzaFirma:!m.firma};
  const mostra=()=>{
    const doc=docDichiarazione(m,c,{data:st.data,testo:st.testo,firmaSrc:st.firmaSrc,senzaFirma:st.senzaFirma});
    const ant=anteprimaStampa({titolo:m.nome+(c?' · '+c.nome:''),doc,riferimento:c?c.nome:'',registra:false,
      azioniExtra:`<button class="pulsante" data-dich-modifica>${icona('modifica')}Modifica</button><button class="pulsante primario" data-dich-salva>${icona('ok')}Salva</button>`});
    const bm=el('[data-dich-modifica]');
    if(bm) bm.onclick=async()=>{ ant.chiudi(); if(await modificaDichiarazione(st,m)) mostra(); else mostra(); };
    const bs=el('[data-dich-salva]');
    if(bs) bs.onclick=()=>{ ant.chiudi(); salvaDichiarazione(m,c,st,ant.pagine); };
  };
  mostra();
}
async function modificaDichiarazione(st,m){
  const firme=firmeDisponibili();
  const v=await dialogoModulo('Modifica prima di salvare',[
    {nome:'data',etichetta:'Data',tipo:'data',obbligatorio:true},
    {nome:'firma',etichetta:'Firma da usare',tipo:'select',vuoto:false,opzioni:[{v:'',t:'Quella prevista dal modello'},{v:'nessuna',t:'Nessuna firma (si firma a mano)'},...firme.map(f=>({v:f.id,t:f.nome}))]},
    {nome:'testo',etichetta:'Testo della dichiarazione',tipo:'textarea',righe:16,largo:true,obbligatorio:true,aiuto:'Vale solo per questa compilazione: il modello non viene toccato'},
  ],{data:st.data,firma:st.firmaId||'',testo:st.testo},{ok:'Rivedi l\'anteprima'});
  if(!v) return false;
  st.data=v.data;st.testo=v.testo;st.firmaId=v.firma;
  if(v.firma==='nessuna'){st.senzaFirma=true;st.firmaSrc=null;st.firmaNome='nessuna'}
  else if(v.firma){const f=firme.find(x=>x.id===v.firma);st.senzaFirma=false;st.firmaSrc=f?f.src:null;st.firmaNome=f?f.nome:''}
  else {st.senzaFirma=!m.firma;st.firmaSrc=null;st.firmaNome='quella prevista dal modello'}
  return true;
}
function salvaDichiarazione(m,c,st,pagine){
  const g={id:nuovoId('g'),titolo:m.nome+(c?' · '+c.nome:''),quando:new Date().toISOString(),pagine,orientamento:'verticale',riferimento:c?c.nome:''};
  esegui('Salvata dichiarazione: '+m.nome,s=>{
    s.generati.push(g);if(s.generati.length>200)s.generati.shift();
    if(c){const cc=s.cantieri.find(x=>x.id===c.id);cc.documentiProdotti.push({id:nuovoId('dp'),tipo:'Dichiarazione',titolo:m.nome,data:st.data||oggi(),modelloId:m.id,generatoId:g.id,fileId:null,note:(st.firmaNome&&st.firmaNome!=='nessuna'?'Firma applicata: '+st.firmaNome+'. ':'')+'Se la committenza richiede firma digitale, il PDF va firmato con dispositivo.'});}
  });
  if(c){ ui.filtri['ling:cantiere:'+c.id]='checklist'; vai('cantieri/'+c.id); avviso('«'+m.nome+'» salvata: nella checklist risulta fatta.'); }
