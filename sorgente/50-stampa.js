...
function docLibroPresenze(anno,mese){
  ...
  const testa=`<thead><tr><th style="width:26mm">Persona</th><th style="width:16mm"></th>${giorni.map(g=>`<th class="${cls(g)}">${g}<br><span style="font-weight:400">${NOMI_GIORNI_BREVI[giornoSettimana(anno,mese,g)].slice(0,1)}</span></th>`).join('')}<th style="width:10mm">Ore</th><th style="width:14mm">Importo</th></tr></thead>`;
  const righe=[];
  for(const sz of ['soci','dipendenti']){const pp=persone.filter(p=>(p.sezionePresenze||'dipendenti')===sz);if(!pp.length)continue;righe.push(`<tr><td colspan="${n+4}" style="background:#D9D9D9;font-weight:700;text-align:left">${sz==='soci'?'Soci':'Dipendenti'}</td></tr>`);
    for(const p of pp){const mp=m.persone[p.id]||{giorni:{}};const calc=calcolaMesePersona(mp,p);const cella=(g,riga)=>{const c=mp.giorni[String(g)]||{};if(riga==='ore'){const v=valoreCella(c);return v==null?'':typeof v==='number'?fOre(v):v}return c[riga]||''};
      if(p.soloTrasferte){righe.push(`<tr><td class="nome">${h(nomePersona(p))}</td><td class="sub">trasferte</td>${giorni.map(g=>`<td class="${cls(g)}">${h(cella(g,'trasferta'))}</td>`).join('')}<td></td><td class="num">${h(fEuro(calc.importo,0))}</td></tr>`);continue}
      righe.push(`<tr><td class="nome">${h(nomePersona(p))}</td><td class="sub">ore</td>${giorni.map(g=>`<td class="${cls(g)}"><b>${h(cella(g,'ore'))}</b></td>`).join('')}<td class="num">${h(fOre(calc.oreGriglia))}</td><td class="num">${calc.importo!=null?h(fEuro(calc.importo,0)):''}</td></tr>`);
      righe.push(`<tr><td></td><td class="sub">cantiere</td>${giorni.map(g=>`<td class="${cls(g)}">${h(tronca(cella(g,'cantiere'),9))}</td>`).join('')}<td colspan="2" class="sub">${h(Object.entries(calc.perCodice).map(([c,nn])=>c+' '+nn).join(' '))}</td></tr>`);
      righe.push(`<tr><td></td><td class="sub">committente</td>${giorni.map(g=>`<td class="${cls(g)}">${h(tronca(cella(g,'committente'),9))}</td>`).join('')}<td colspan="2"></td></tr>`);
      if(mp.note)righe.push(`<tr><td></td><td class="sub">note</td><td colspan="${n+2}" class="sub" style="text-align:left;white-space:normal">${h(mp.note)}</td></tr>`);}}
  const rie=riepilogoMese(anno,mese);
  const blocchi=[`<h1 class="sx">Libro presenze — ${h(fMeseAnno(anno,mese))}</h1>`,`<table class="libro-presenze">${testa}<tbody>${righe.join('')}</tbody></table>`,`<p class="mini sx">Codici: M malattia · I infortunio · PE permesso · FS festività · FE ferie · AS assenza · CI cassa integrazione. Sabato e domenica non si compilano. Totale ore in griglia ${h(fOre(rie.oreTotali))} · importo complessivo ${h(fEuro(rie.importoTotale,0))}.</p>`];
