...
VISTE.presenze=function(r){
  ...
  // avvisi: massimali annui, differenze fra dichiarate e griglia
  const avvisi=[];
  for(const p of persone){const mesiAnno=Object.keys(stato.presenze).filter(x=>x.startsWith(anno+'-')).map(x=>stato.presenze[x].persone[p.id]).filter(Boolean);const mx=controllaMassimali(mesiAnno);for(const a of mx.avvisi)avvisi.push(nomePersona(p)+': '+a);const mp=m.persone[p.id];if(mp){const c=calcolaMesePersona(mp,p);if(c.differenzaOre)avvisi.push(`${nomePersona(p)}: ore dichiarate ${fOre(c.oreDichiarate)} contro ${fOre(c.oreGriglia)} in griglia (${c.differenzaOre>0?'+':''}${fOre(c.differenzaOre)})`);if(mp.incerti&&mp.incerti.length)avvisi.push(`${nomePersona(p)}: ${mp.incerti.length} giorni letti con incertezza da confermare (${mp.incerti.join(', ')})`)}}
  ...
  <div class="presenze-griglia desktop ${compatta?'compatta':''}" id="presenze-griglia" tabindex="0" data-k="${k}"><table>
    <thead><tr><th class="fisso">Persona</th><th class="fisso2"></th>${giorni.map(d=>html`<th class="${d.we?'fine-settimana':''} ${d.festivo?'festivo':''}" title="${fData(d.iso)}${d.festivo?' · festività':''}" ${d.oggi?'style="outline:2px solid var(--blu);outline-offset:-2px"':''}><span class="gs">${NOMI_GIORNI_BREVI[d.gs]}</span>${d.g}</th>`)}<th>Ore</th><th>Dichiarate</th><th>Importo</th></tr></thead>
    <tbody>${sezioni.map(([sz,tit])=>{const pp=persone.filter(p=>(p.sezionePresenze||'dipendenti')===sz);if(!pp.length)return '';return html`<tr class="sezione"><td colspan="${n+5}">${tit}</td></tr>${pp.map(p=>righePersona(p,m.persone[p.id]||{giorni:{}},giorni,k))}`})}</tbody></table></div>
...
function righePersona(p,mp,giorni,k){
  ...
  if(p.soloTrasferte){
    return html`<tr><td class="fisso"><a href="#/operai/${p.id}">${nomePersona(p)}</a></td><td class="fisso2">trasferte</td>${giorni.map(d=>cella(d,'trasferta'))}<td class="totale">—</td><td class="totale">—</td><td class="totale num" data-azione="presenze-persona" data-k="${k}" data-pid="${p.id}" style="cursor:pointer" title="Apri il riepilogo">${fEuro(calc.importo,0)}${mp.importoForzato?' ✎':''}</td></tr>`;
  }
  return html`<tr><td class="fisso"><a href="#/operai/${p.id}">${nomePersona(p)}</a></td><td class="fisso2">ore</td>${giorni.map(d=>cella(d,'ore'))}<td class="totale num">${fOre(calc.oreGriglia)}</td><td class="totale num ${calc.differenzaOre?'da-compilare':''}" data-azione="presenze-persona" data-k="${k}" data-pid="${p.id}" style="cursor:pointer" title="Ore dichiarate sul foglio cartaceo">${calc.oreDichiarate!=null?fOre(calc.oreDichiarate):'—'}</td><td class="totale num" data-azione="presenze-persona" data-k="${k}" data-pid="${p.id}" style="cursor:pointer" title="Apri il riepilogo della persona">${calc.importo!=null?fEuro(calc.importo,0):'—'}${mp.importoForzato?' ✎':''}</td></tr>
  <tr class="riga-secondaria"><td class="fisso"></td><td class="fisso2">cantiere</td>${giorni.map(d=>cella(d,'cantiere'))}<td class="totale" colspan="3">${Object.entries(calc.perCodice).map(([c,nn])=>html`<span class="etichetta-tag" title="${(CODICI_ASSENZA[c]||{}).nome||c}">${c} ${nn}</span> `)}</td></tr>
  <tr class="riga-secondaria"><td class="fisso"></td><td class="fisso2">committente</td>${giorni.map(d=>cella(d,'committente'))}<td class="totale" colspan="3">${mp.foglioOreId?html`<button class="pulsante piccolo" data-azione="file-apri" data-id="${mp.foglioOreId}">${icona('immagine','piccola')}foglio</button>`:''}${(mp.aggiustamenti||[]).length?html` <span class="piccolo secondario">${mp.aggiustamenti.map(a=>a.sigla+' '+fEuro(a.importo,0)).join(', ')}</span>`:''}</td></tr>`;
}
...
function vistaPresenzeMobile(persone,m,giorni,k){
  ...
  <div class="riga mt-s"><span>Ore: <b>${fOre(calc.oreGriglia)}</b></span><span>Dich.: <b>${calc.oreDichiarate!=null?fOre(calc.oreDichiarate):'—'}</b></span><span class="spazio"></span><b>${fEuro(calc.importo,0)}</b><button class="pulsante piccolo" data-azione="presenze-persona" data-k="${k}" data-pid="${p.id}">Riepilogo</button></div>
...
// ---- riepilogo persona: ore dichiarate, aggiustamenti, importo forzato, foglio ore, note ----
AZIONI['presenze-persona']=async d=>{
  ...
  const v=await dialogoModulo(nomePersona(p)+' · '+fMeseAnno(anno,mese),[
    {nome:'oreDichiarate',etichetta:'Ore realmente dichiarate sul foglio',tipo:'ore',aiuto:'Il totale scritto a mano in fondo al foglio. Può differire dalla griglia ('+fOre(calc.oreGriglia)+'): il lavoro nel fine settimana conta qui, non in griglia.'},
    {nome:'aggiustamentiTesto',etichetta:'Aggiustamenti (una riga: sigla importo)',tipo:'textarea',segnaposto:'brc 50\nmt -20',aiuto:'Anche negativi. Contano solo gli importi.'},
  ...
  ],{oreDichiarate:mp.oreDichiarate,aggiustamentiTesto:agg,importoForzato:!!mp.importoForzato,importoManuale:mp.importoManuale,note:mp.note||''},{intro:html`<div class="griglia tre mb"><div class="indicatore" style="cursor:default"><span class="etichetta">Ore in griglia</span><span class="valore md">${fOre(calc.oreGriglia)}</span></div><div class="indicatore" style="cursor:default"><span class="etichetta">Importo calcolato</span><span class="valore md">${fEuro(calc.importoCalcolato,0)}</span><span class="nota">${calc.tariffa?fOre(calc.oreBase)+' h × '+fNum(calc.tariffa,2)+' € = '+fEuro(calc.importoBase):''}${calc.fisso?' + fisso '+fEuro(calc.fisso,0):''}${calc.aggiustamenti?' + agg. '+fEuro(calc.aggiustamenti,0):''}</span></div><div class="indicatore" style="cursor:default"><span class="etichetta">Foglio ore</span><span class="valore md" style="font-size:14px">${mp.foglioOreId?html`<button class="pulsante piccolo" data-azione="file-apri" data-id="${mp.foglioOreId}">${icona('immagine','piccola')}Vedi</button>`:html`<span class="secondario">nessuno</span>`} <button class="pulsante piccolo" data-azione="presenze-foglio-carica" data-k="${d.k}" data-pid="${p.id}">${icona('fotocamera','piccola')}Foto</button></span></div></div>`});
  if(!v) return;
  ...
  esegui('Riepilogo '+nomePersona(p)+' '+fMeseAnno(anno,mese),s=>{const x=assicuraMesePersona(s,anno,mese,p.id);x.oreDichiarate=v.oreDichiarate;x.aggiustamenti=aggiustamenti;x.importoForzato=!!v.importoForzato;x.importoManuale=v.importoForzato?v.importoManuale:null;x.note=v.note});
};
...
async function leggiFoglioConAi(fileId,anno,mese){
  ...
  const richiesta=`Questa è la foto di un foglio ore scritto a mano di un operaio edile per ${NOMI_MESI[mese-1]} ${anno}. Trascrivi solo ciò che leggi con certezza, senza inventare nulla. Rispondi con un solo oggetto JSON, senza testo attorno, nella forma {"giorni":{"1":{"ore":8,"cantiere":"nome scritto"},"2":{"codice":"FE"}},"oreDichiarate":168}. Codici ammessi: M (malattia), I (infortunio), PE (permesso), FS (festività), FE (ferie), AS (assenza), CI (cassa integrazione). Ometti i giorni vuoti o illeggibili, i sabati e le domeniche. Ometti oreDichiarate se sul foglio non c'è un totale scritto.`;
  ...
  return {giorni:dati.giorni&&typeof dati.giorni==='object'?dati.giorni:{},oreDichiarate:dati.oreDichiarate!=null&&isFinite(+dati.oreDichiarate)?+dati.oreDichiarate:null};
}
async function apriTrascrizione(anno,mese,pid){
  ...
  <div class="riga mt-s piccolo secondario"><span class="kbd">Invio</span> giù nella stessa colonna · <span class="kbd">Tab</span> a destra · <span class="kbd">⌘D</span> copia dalla riga sopra · <span class="kbd">⌘↓</span> riempi in giù fino a fine mese · <span class="kbd">?</span> segna incerto</span><span class="spazio"></span><label class="riga stretta">Ore dichiarate sul foglio <input type="text" id="tr-dich" value="${mp.oreDichiarate!=null?fOre(mp.oreDichiarate):''}" style="width:80px" inputmode="decimal"></label></div>`;
  const ris=await dialogo({titolo:'Trascrizione · '+nomePersona(p)+' · '+fMeseAnno(anno,mese),enorme:true,corpo,senzaFocus:true,valoreEscape:null,pulsanti:[{testo:'Annulla',valore:null},{testo:'Applica',classe:'primario',primario:true,fn:v=>leggiTrascrizione(v)}],alMontaggio:v=>{
    ...
    const ai=v.querySelector('#tr-ai');if(ai)ai.onclick=async()=>{ai.disabled=true;const ih=ai.innerHTML;ai.textContent='Lettura in corso…';try{const prop=await leggiFoglioConAi(mp.foglioOreId,anno,mese);let n=0;for(const [g,c] of Object.entries(prop.giorni||{})){const ore=v.querySelector(`input[data-tr="ore"][data-g="${g}"]`);if(!ore||!c)continue;const val=c.codice?String(c.codice).toUpperCase():(c.ore!=null&&isFinite(+c.ore)?fOre(+c.ore):'');if(!val)continue;ore.value=val;const ca=v.querySelector(`input[data-tr="cantiere"][data-g="${g}"]`);if(ca&&c.cantiere)ca.value=String(c.cantiere);const inc=v.querySelector(`input[data-tr="incerto"][data-g="${g}"]`);if(inc)inc.checked=true;n++}const dich=v.querySelector('#tr-dich');if(prop.oreDichiarate!=null&&!dich.value)dich.value=fOre(prop.oreDichiarate);avviso(n?`Proposte AI per ${n} giorni, tutte segnate come incerte: confrontale con la foto prima di applicare`:'Il servizio non ha riconosciuto giorni compilabili',{tipo:'attenzione'})}catch(e){avviso('Lettura assistita non riuscita: '+String(e&&e.message||e),{tipo:'errore'})}finally{ai.disabled=false;ai.innerHTML=ih}};
  ...
  esegui('Trascrizione '+nomePersona(p)+' '+fMeseAnno(anno,mese),s=>{
    ...
    x.oreDichiarate=ris.dichiarate;
  });
