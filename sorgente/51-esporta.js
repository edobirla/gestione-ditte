...
async function creaXlsx(fogli){
  ...
  voci.push({nome:'docProps/core.xml',dati:`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"><dc:creator>Gestionale</dc:creator><dcterms:created xsi:type="dcterms:W3CDTF">${new Date().toISOString()}</dcterms:created></cp:coreProperties>`});
  voci.push({nome:'docProps/app.xml',dati:`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties"><Application>Gestionale</Application></Properties>`});
...
function indiceCantieriMarkdown(){
  const riga=c=>`- **${c.anno}/${c.nome}** – ${nomeCliente(c.affidatariaId)?stato.azienda.ragioneSociale+' subcontracted by '+nomeCliente(c.affidatariaId):''}${nomeCliente(c.committenteId)?' | Committente: '+nomeCliente(c.committenteId):''}${c.dataInizio?' | Start: '+fData(c.dataInizio):''}${c.dataFine?' | End: '+fData(c.dataFine):''}${!c.dataInizio&&c.periodoTesto?' | '+c.periodoTesto:''}${stato.pos.filter(p=>p.cantiereId===c.id).length?' | POS rev. '+Math.max(...stato.pos.filter(p=>p.cantiereId===c.id).map(p=>p.revisione)):''}\n`;
...
AZIONI['esporta-schema']=()=>mostraTestoEsportato('Documentazione dello schema dati',documentazioneSchema(),nomeFileData('Schema dati gestionale','md'));
...
AZIONI['pacchetto-genera']=async d=>{
  ...
  try{
    ...
    const blob=await creaZip(voci);
    prog.chiudi();
    const nome=nomeFileData('Documenti '+pulisci(c.nome)+' '+pulisci(stato.azienda.ragioneSociale||''),'zip');
    ...
    ui.pacchetto=null;
    const esito=await condividiOScarica(blob,nome,{testo:'Documentazione '+(stato.azienda.ragioneSociale||'')+' per il cantiere '+c.nome});
