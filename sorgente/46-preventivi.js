...
VISTE.preventivi=function(r){
  ...
  return html`<div class="testata"><div><h1>Preventivi</h1><div class="sotto">${stato.preventivi.length} preventivi · ${stato.listino.length} voci di listino</div></div><div class="azioni"><button class="pulsante" data-azione="preventivo-importa-voci" title="Carica il capitolato o il computo del cliente (Excel o PDF): le voci finiscono in un preventivo Pavimass">${icona('carica')}Importa voci dal cliente</button><a class="pulsante" href="#/preventivi/listino">${icona('elenco')}Listino</a><button class="pulsante primario" data-azione="preventivo-nuovo">${icona('piu')}Nuovo preventivo</button></div></div>
  ${tabella({id:'preventivi',righe,href:x=>'#/preventivi/'+x.p.id,colonne:[{chiave:'n',titolo:'Numero',principale:true,valore:x=>(x.p.anno*1000+x.p.numero),formatta:x=>html`<b>${x.p.numero}/${x.p.anno}</b>`},{chiave:'data',titolo:'Data',valore:x=>x.p.data,formatta:x=>fData(x.p.data)},{chiave:'cl',titolo:'Cliente',valore:x=>nomeCliente(x.p.clienteId),formatta:x=>nomeCliente(x.p.clienteId)||daCompilare()},{chiave:'ogg',titolo:'Oggetto',valore:x=>x.p.oggetto||''},{chiave:'stato',titolo:'Stato',valore:x=>x.p.stato,formatta:x=>html`<select class="stato-rapido ${x.p.stato}" data-cambio="preventivo-stato" data-id="${x.p.id}" aria-label="Stato del preventivo" onclick="event.stopPropagation()">${Object.entries(STATI_PREVENTIVO).map(([v,t])=>html`<option value="${v}" ${x.p.stato===v?'selected':''}>${t}</option>`)}</select>`},{chiave:'dq',titolo:'Da quotare',num:true,valore:x=>x.tot.daQuotare.length,formatta:x=>x.tot.daQuotare.length?html`<span class="pillola scadenza">${icona('attenzione','piccola')}${x.tot.daQuotare.length}</span>`:''},{chiave:'tot',titolo:'Totale',num:true,valore:x=>x.tot.totale,formatta:x=>fEuro(x.tot.totale)}],vuoto:vuoto({icona:'preventivi',titolo:'Nessun preventivo',testo:'Crea il primo preventivo dal listino o incollando il computo del cliente.',azione:{testo:'Nuovo preventivo',azione:'preventivo-nuovo'}})})}`;
};
...
function vistaPreventivo(id){
  ...
  <div class="testata"><div><h1>Preventivo ${p.numero}/${p.anno}</h1><div class="sotto"><span class="gruppo-pulsanti stati-prev">${Object.entries(STATI_PREVENTIVO).map(([v,t])=>html`<button class="pulsante piccolo ${p.stato===v?'attivo '+v:''}" data-azione="preventivo-stato-imposta" data-id="${p.id}" data-valore="${v}">${t}</button>`)}</span> ${fData(p.data)} · ${nomeCliente(p.clienteId)||daCompilare('cliente')}${p.cantiereId?html` · <a href="#/cantieri/${p.cantiereId}">${nomeCantiere(p.cantiereId)}</a>`:''}</div></div>
...
// ---- estrazione delle voci dal capitolato del cliente nel modello Pavimass ----
// Scelta di Edoardo: le voci del file del cliente (Excel o PDF) vengono portate dentro un preventivo
// Pavimass, che esce sempre col nostro template. Il file del cliente non viene modificato.
