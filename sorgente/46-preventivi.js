...
VISTE.preventivi=function(r){
  ...
  return html`<div class="testata"><div><h1>Preventivi</h1><div class="sotto">${stato.preventivi.length} preventivi · ${stato.listino.length} voci di listino</div></div><div class="azioni"><button class="pulsante" data-azione="preventivo-compila-excel" title="Carica il modulo Excel ricevuto dal cliente e compilalo, mantenendo il suo formato originale">${icona('carica')}Compila modulo del cliente</button><a class="pulsante" href="#/preventivi/listino">${icona('elenco')}Listino</a><button class="pulsante primario" data-azione="preventivo-nuovo">${icona('piu')}Nuovo preventivo</button></div></div>
...
// ---- compilazione di un modulo Excel ricevuto dal cliente, mantenendo il suo formato esatto ----
// Su richiesta di Edoardo: spesso il cliente vuole il preventivo nel SUO modulo, non nel nostro.
// Si modificano solo le celle toccate nell'XML originale del foglio: tutto il resto (impaginazione,
// stili, altri fogli, formule) resta byte per byte quello che era.
function indiceColLettere(lettere){let n=0;for(const ch of lettere)n=n*26+(ch.charCodeAt(0)-64);return n-1}
function rifCellaXlsx(riga,col){return colXlsx(col)+(riga+1)}
AZIONI['preventivo-compila-excel']=async()=>{
  const fs=await scegliFile({multipli:false,accetta:'.xlsx'});
  if(!fs.length) return;
  await apriCompilatoreExcel(fs[0]);
};
async function apriCompilatoreExcel(file){
  let zip; try{ zip=await leggiZip(file); }catch(e){ return segnalaErrore(e,'Non sono riuscito ad aprire il file: dev\'essere un .xlsx'); }
  const vSheet=zip.voci.find(v=>v.nome==='xl/worksheets/sheet1.xml');
  if(!vSheet) return avviso('Non ho trovato un foglio di lavoro leggibile in questo file',{tipo:'errore'});
  const xmlTesto=await zip.testo(vSheet);
  ...
  if(doc.querySelector('parsererror')) return avviso('Il foglio di lavoro non è leggibile (XML non valido)',{tipo:'errore'});
  let righeMax=0,colMax=0;
  const dimRef=(doc.querySelector('dimension')||{}).getAttribute&&doc.querySelector('dimension').getAttribute('ref');
  const mDim=dimRef&&/^[A-Z]+\d+:([A-Z]+)(\d+)$/.exec(dimRef);
  if(mDim){colMax=indiceColLettere(mDim[1]);righeMax=+mDim[2]-1}
  const celle=new Map();
  tutti('c',doc).forEach(c=>{
    ...
    const t=c.getAttribute('t'); const vEl=c.querySelector('v'); const isEl=c.querySelector('is');
    let testo='';
    if(t==='s'&&vEl) testo=shared[+vEl.textContent]||'';
    else if(isEl) testo=(isEl.textContent||'').trim();
    else if(vEl) testo=vEl.textContent;
    celle.set(ref,testo);
  });
  righeMax=Math.min(righeMax,300); colMax=Math.min(colMax,40);
  const modifiche=new Map();
  const griglia=()=>{
    let intestazione='<th></th>';for(let c=0;c<=colMax;c++)intestazione+=`<th>${colXlsx(c)}</th>`;
    let corpo='';
    for(let r=0;r<=righeMax;r++){
      let riga=`<th>${r+1}</th>`;
      for(let c=0;c<=colMax;c++){const ref=rifCellaXlsx(r,c);const valore=modifiche.has(ref)?modifiche.get(ref):(celle.get(ref)||'');riga+=`<td><input type="text" data-cella="${ref}" value="${h(valore)}"></td>`}
      corpo+=`<tr>${riga}</tr>`;
    }
    return `<div class="tabella-scroll"><table class="tabella-excel"><thead><tr>${intestazione}</tr></thead><tbody>${corpo}</tbody></table></div>`;
  };
  const risultato=await dialogo({titolo:'Compila «'+file.name+'»',enorme:true,
    corpo:html`<p class="piccolo secondario">Il file resta nel suo formato originale: cambia solo le celle che tocchi, il resto (impaginazione, altri fogli, formule) resta identico.</p>${grezzo(griglia())}`,
    pulsanti:[{testo:'Annulla',valore:null},{testo:'Scarica il file compilato',classe:'primario',primario:true,fn:()=>true}],
    alMontaggio:v=>{v.addEventListener('input',e=>{const t=e.target;if(t.dataset&&t.dataset.cella)modifiche.set(t.dataset.cella,t.value)})}});
  if(!risultato) return;
  if(!modifiche.size) return avviso('Nessuna cella modificata: niente da scaricare');
  for(const [ref,valore] of modifiche){
    let c=doc.querySelector(`c[r="${ref}"]`);
    if(!c){
      const m=/^([A-Z]+)(\d+)$/.exec(ref); const rNum=+m[2]; const colIdx=indiceColLettere(m[1]);
      let rowEl=doc.querySelector(`row[r="${rNum}"]`);
      if(!rowEl){
        rowEl=doc.createElementNS(doc.documentElement.namespaceURI,'row'); rowEl.setAttribute('r',String(rNum));
        const sheetData=doc.querySelector('sheetData'); let inserito=false;
        for(const rw of Array.from(sheetData.children)){ if(+rw.getAttribute('r')>rNum){sheetData.insertBefore(rowEl,rw);inserito=true;break} }
        if(!inserito) sheetData.appendChild(rowEl);
      }
      c=doc.createElementNS(doc.documentElement.namespaceURI,'c'); c.setAttribute('r',ref);
      let inserito=false;
      for(const cc of Array.from(rowEl.children)){ const ccM=/^([A-Z]+)\d+$/.exec(cc.getAttribute('r')); if(ccM&&indiceColLettere(ccM[1])>colIdx){rowEl.insertBefore(c,cc);inserito=true;break} }
      if(!inserito) rowEl.appendChild(c);
    }
    while(c.firstChild) c.removeChild(c.firstChild);
    const numerico=valore.trim()!==''&&/^-?[\d.,]+$/.test(valore.trim())&&leggiNumero(valore)!==null;
    if(valore===''){ c.removeAttribute('t'); }
    else if(numerico){ c.removeAttribute('t'); const vEl=doc.createElementNS(doc.documentElement.namespaceURI,'v'); vEl.textContent=String(leggiNumero(valore)); c.appendChild(vEl); }
    else { c.setAttribute('t','inlineStr'); const isEl=doc.createElementNS(doc.documentElement.namespaceURI,'is'); const tEl=doc.createElementNS(doc.documentElement.namespaceURI,'t'); tEl.setAttribute('xml:space','preserve'); tEl.textContent=valore; isEl.appendChild(tEl); c.appendChild(isEl); }
  }
  const nuovoXml=new XMLSerializer().serializeToString(doc);
  const nuoveVoci=[];
  for(const v of zip.voci) nuoveVoci.push({nome:v.nome,dati:v===vSheet?nuovoXml:await zip.estrai(v)});
  const blob=await creaZip(nuoveVoci);
  const nomeFinale=file.name.replace(/\.xlsx$/i,'')+' - compilato.xlsx';
  await condividiOScarica(new Blob([blob],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),nomeFinale);
}
