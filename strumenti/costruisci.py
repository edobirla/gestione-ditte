...
inclusa mai, e si ottiene il file da dare a chiunque altro.
    ...
    python3 strumenti/costruisci.py --vuoto    -> Gestionale (senza dati).html
"""
import os,re,sys
...
def pezzi(cartella):
    ...
    return [(cartella,p) for p in sorted(os.listdir(d)) if not p.startswith('.')]
...
dest=os.path.join(base,'Gestionale (senza dati).html' if vuoto else 'Gestionale Pavimass.html')
